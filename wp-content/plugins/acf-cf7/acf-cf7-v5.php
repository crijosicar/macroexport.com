	...
	function format_value( $value, $post_id, $field ) {
		...
		if(is_array($value)){
		    foreach($value as $k => $v){
		      	$form = get_post($v);
		      	$f = do_shortcode('[contact-form-7 id="'.$form->ID.'" title="'.$form->post_title.'" html_id="form_'.$form->ID.'" ]');
		      	$value[$k] = array();
		      	$value[$k] = $f;
		    }
		//Else return single form markup
		    ...
		    $form = get_post($value);
		    $value = do_shortcode('[contact-form-7 id="'.$form->ID.'" title="'.$form->post_title.' html_id="form_'.$form->ID.'"]');
		}
